<x-layouts::cliente :title="__('Mis solicitudes')">
    @php
        ...
        $statusLabel = fn (\App\Domain\Needs\Models\Need $need) => match ($need->status) {
        ...
        $statusClasses = fn (\App\Domain\Needs\Models\Need $need) => match ($need->status) {
            \App\Domain\Needs\Models\Need::BORRADOR => 'bg-zinc-100 text-zinc-700',
            \App\Domain\Needs\Models\Need::PUBLICADA, \App\Domain\Needs\Models\Need::RECIBIENDO_OFERTAS => 'bg-emerald-100 text-emerald-700',
            \App\Domain\Needs\Models\Need::SELECCIONADA => 'bg-sky-100 text-sky-700',
            default => 'bg-rose-100 text-rose-700',
        };

        $cardIcon = fn (\App\Domain\Needs\Models\Need $need) => $need->category ? 'cake' : 'document-text';
        $cardIconShell = fn (\App\Domain\Needs\Models\Need $need) => $need->isDraft()
    ...
    <div class="mx-auto max-w-6xl px-6 py-10 sm:px-8 lg:px-10">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-3">
                <h1 class="text-xl font-semibold tracking-tight text-zinc-950 dark:text-white sm:text-2xl">
                    {{ __('Mis solicitudes') }}
                </h1>
                <p class="max-w-xl text-md leading-8 text-zinc-500 dark:text-zinc-400">
            ...
            <a
                ...
                wire:navigate
                class="inline-flex items-center justify-center gap-3 rounded-xl bg-brand-600 px-3 py-2 text-sm font-semibold text-white shadow-[0_18px_40px_rgba(220,38,38,0.22)] transition hover:bg-brand-700"
            >
        ...
        <div class="mt-4 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.06)] dark:border-zinc-800 dark:bg-zinc-900">
            <div class="grid gap-0 md:grid-cols-3">
                @foreach ($summaryCards as $card)
                    <div class="flex items-center gap-4 px-5 py-4 {{ ! $loop->last ? 'border-b border-zinc-200 md:border-b-0 md:border-r dark:border-zinc-800' : '' }}">
                        <span class="flex size-14 shrink-0 items-center justify-center rounded-lg {{ $card['shell'] }}">
                            <flux:icon :name="$card['icon']" class="size-7" variant="outline" />
                        </span>
                        <div class="text-md font-medium tracking-tight text-zinc-700 dark:text-zinc-200">
                            {{ $card['value'] }}
                        </div>
                    </div>
                @endforeach
        ...
        <div class="mt-8 inline-flex w-full flex-wrap overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:w-auto">
            @foreach ($tabs as $tabKey => $tabLabel)
                ...
                <a
                    ...
                    wire:navigate
                    class="inline-flex min-w-40 items-center justify-center gap-2 border-r border-zinc-200 px-2 py-2 text-sm font-medium transition last:border-r-0 dark:border-zinc-800 {{ $isActive ? 'bg-brand-50 text-brand-700' : 'text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-white' }}"
                >
                    <span>{{ $tabLabel }}</span>
        ...
        <section class="mt-6">
            <h2 class="text-xl font-semibold tracking-tight text-zinc-950 dark:text-white">
                {{ __('Tus solicitudes') }}
            </h2>
            ...
            @if ($filteredNeeds->isEmpty())
                <div class="mt-5 rounded-[28px] border border-zinc-200 bg-white px-8 py-12 text-center shadow-[0_18px_60px_rgba(15,23,42,0.06)] dark:border-zinc-800 dark:bg-zinc-900">
                    ...
                    <h3 class="mt-5 text-lg font-semibold text-zinc-900 dark:text-white">
                        {{ __('No hay solicitudes en esta vista') }}
                    </h3>
                    <p class="mx-auto mt-3 max-w-xl text-sm leading-8 text-zinc-500 dark:text-zinc-400">
            ...
            @else
                <div class="mt-5 space-y-4">
                    @foreach ($filteredNeeds as $need)
                        ...
                        <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-[0_18px_60px_rgba(15,23,42,0.06)] dark:border-zinc-800 dark:bg-zinc-900 sm:p-5">
                            <div class="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                                <div class="flex min-w-0 items-start gap-4">
                                    <span class="flex size-15 shrink-0 items-center justify-center rounded-xl {{ $cardIconShell($need) }}">
                                        <flux:icon :name="$cardIcon($need)" class="size-10" variant="outline" />
                                    </span>

                                    <div class="min-w-0 space-y-1.5">
                                        <h3 class="truncate text-lg font-semibold tracking-tight text-zinc-950 dark:text-white">
                                            {{ $title }}
                                        </h3>
                                        <p class="text-sm text-zinc-600 dark:text-zinc-300">{{ $subtitle }}</p>

                                        @if ($meta)
                                            <p class="text-sm text-zinc-400 dark:text-zinc-500">{{ $meta }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col gap-4 xl:min-w-[520px] xl:flex-row xl:items-center xl:justify-end">
                                    ...
                                    <span class="inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-medium {{ $statusClasses($need) }}">
                                        {{ $statusLabel($need) }}
                                    </span>
                                    ...
                                    <a
                                        ...
                                        wire:navigate
                                        class="inline-flex min-w-40 items-center justify-center rounded-xl border border-brand-300 px-2 py-2 text-sm font-medium text-brand-700 transition hover:bg-brand-50 dark:border-brand-700/60 dark:text-brand-300 dark:hover:bg-brand-500/10"
                                    >
                                        {{ $cta }}
                                    </a>
                                </div>
        ...
        <div class="mt-12 flex items-center justify-center gap-3 text-center text-sm text-zinc-500 dark:text-zinc-400">
